from __future__ import annotations

from datetime import datetime, timedelta, timezone

import pytest
from httpx import AsyncClient
from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from app.models.extracted_result import ExtractedResult
from app.models.processing_job import ProcessingJob


async def _seed_completed_result(
    test_db: AsyncSession,
    document_id: str,
    job_id: str,
    *,
    finalized: bool = False,
) -> None:
    job = await test_db.scalar(select(ProcessingJob).where(ProcessingJob.id == job_id))
    assert job is not None

    job.status = "completed"
    job.progress_percentage = 100
    job.completed_at = datetime.now(timezone.utc)

    extracted = ExtractedResult(
        document_id=document_id,
        job_id=job.id,
        word_count=95,
        readability_score=68.2,
        primary_keywords=[{"keyword": "content", "count": 4, "density": 4.2}],
        auto_summary="Auto summary available",
        user_edited_summary=None,
        is_finalized=finalized,
        finalized_at=datetime.now(timezone.utc) if finalized else None,
    )
    test_db.add(extracted)
    await test_db.flush()


@pytest.mark.asyncio
async def test_finalize_success(
    client: AsyncClient,
    auth_headers: dict[str, str],
    create_document_for_user,
    test_db: AsyncSession,
) -> None:
    upload = await create_document_for_user(auth_headers)
    await _seed_completed_result(test_db, upload["document_id"], upload["job_id"])

    response = await client.post(
        f"/api/v1/documents/{upload['document_id']}/finalize",
        headers=auth_headers,
    )

    assert response.status_code == 200
    data = response.json()
    assert data["is_finalized"] is True
    assert data["finalized_at"] is not None


@pytest.mark.asyncio
async def test_finalize_already_finalized(
    client: AsyncClient,
    auth_headers: dict[str, str],
    create_document_for_user,
    test_db: AsyncSession,
) -> None:
    upload = await create_document_for_user(auth_headers)
    await _seed_completed_result(test_db, upload["document_id"], upload["job_id"])

    first = await client.post(
        f"/api/v1/documents/{upload['document_id']}/finalize",
        headers=auth_headers,
    )
    assert first.status_code == 200

    second = await client.post(
        f"/api/v1/documents/{upload['document_id']}/finalize",
        headers=auth_headers,
    )
    assert second.status_code == 409


@pytest.mark.asyncio
async def test_finalize_wrong_user(
    client: AsyncClient,
    auth_headers: dict[str, str],
    second_user_auth_headers: dict[str, str],
    create_document_for_user,
    test_db: AsyncSession,
) -> None:
    upload_other = await create_document_for_user(second_user_auth_headers)
    await _seed_completed_result(test_db, upload_other["document_id"], upload_other["job_id"])

    response = await client.post(
        f"/api/v1/documents/{upload_other['document_id']}/finalize",
        headers=auth_headers,
    )

    assert response.status_code == 404


@pytest.mark.asyncio
async def test_finalize_stale_job(
    client: AsyncClient,
    auth_headers: dict[str, str],
    create_document_for_user,
    test_db: AsyncSession,
) -> None:
    upload = await create_document_for_user(auth_headers)
    await _seed_completed_result(test_db, upload["document_id"], upload["job_id"])

    original_job = await test_db.scalar(select(ProcessingJob).where(ProcessingJob.id == upload["job_id"]))
    assert original_job is not None

    newer_job = ProcessingJob(
        document_id=original_job.document_id,
        status="queued",
        progress_percentage=0,
        current_stage="job_queued",
        created_at=(original_job.created_at or datetime.now(timezone.utc)) + timedelta(seconds=1),
    )
    test_db.add(newer_job)
    await test_db.flush()

    response = await client.post(
        f"/api/v1/documents/{upload['document_id']}/finalize",
        headers=auth_headers,
    )

    assert response.status_code == 400
